...
<h2>The Algorithm</h2>
The algorithm builds the entire <i>candidate</i>-solution space (size is <span>\\( \sum_{r=1}^n {n \choose r} = {n \choose 1} + {n \choose 2} + \cdot \cdot \cdot + {n \choose n}= 2^n-1 \\)</span>, where \\(n\\) is the number of starting features) and uses dynamic programming to <i>efficiently</i> search for optimal solutions from it.
